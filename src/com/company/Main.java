...
public class Main extends JPanel implements KeyListener {
    ...
    public static final int COLUM = WIDTH / CELL;
    Moses moses;
    ...
    private boolean changeLevel (String result) {
        if (result.equals("Next Level")) {
            System.out.println("Changing to level: " + (level + 1));  // 印出切換關卡資訊
            level ++;
            if (level == 2) {
        ...
        switch (e.getKeyCode()){
            case KeyEvent.VK_UP :
                if (mosesPoint.y > 1){
                    ...
                    System.out.println("Overlap result: " + result);  // 印出碰撞檢測結果
            ...
            case KeyEvent.VK_D:
                for (int i = mosesPoint.x; i <= COLUM; i++) {
                    for (Sprite s : gameView.getElements()) {
